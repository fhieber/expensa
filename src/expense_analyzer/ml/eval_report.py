...
expense-analyzer-de[report-export]``); :func:`build_pdf` raises a
...
from expense_analyzer.ml.evaluation import AblationResult, EvalResult
...
def _require_deps() -> None:
    ...
    if missing:
        raise RuntimeError(
            ...
            + " ".join(missing)
            + "` (or `pip install expense-analyzer-de[report-export]`)."
        )
...
def _append_settings_appendix(
    ...
    story.append(Paragraph(
        ...
        "(or in <font face='Courier'>~/.expense-analyzer/config.yaml</font>) "
