...
from expensa.accounts import slugify
...
from expensa.features.model_registry import (
    EMBEDDING_MODELS,
    ZEROSHOT_MODELS,
    hf_cache_dir,
    is_downloaded,
...
from expensa.storage.admin import reset_all, reset_data
from expensa.storage.backup import (
    export_database,
    restore_database,
    validate_backup,
)
from expensa.storage.crypto import (
    EncryptionError,
    change_password,
    cipher_version,
    decrypt_file,
...
from expensa.storage.stats import database_overview
...
def _render_encryption(cfg) -> None:
    ...
    if not encryption_available() and not encrypted:
        st.info(
            ...
            "restart the UI (`expensa ui-restart`)."
        )
        return
