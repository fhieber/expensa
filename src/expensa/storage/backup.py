...
class ValidationResult:
    ...
    # True when the source file is a SQLCipher (encrypted) database. Lets
    # the UI know it must collect a password before validating/restoring.
...
def _check_required_tables(conn) -> tuple[dict[str, int], list[str]]:
    """Shared body: assert required tables exist and count their rows."""
    counts: dict[str, int] = {}
    errors: list[str] = []
    rows = conn.execute(
    ...
    if missing:
        errors.append("missing required table(s): " + ", ".join(sorted(missing)))
    for t in sorted(REQUIRED_TABLES & present):
        try:
            n = conn.execute(f"SELECT COUNT(*) FROM {t}").fetchone()[0]
            counts[t] = int(n)
        except Exception as e:  # DatabaseError from either driver
            errors.append(f"count {t}: {e}")
    return counts, errors


def validate_backup(source_path: Path, password: str | None = None) -> ValidationResult:
    """Inspect ``source_path`` and report whether it's a usable backup.

    Handles both plaintext SQLite and encrypted SQLCipher backups. For an
    encrypted file a ``password`` is required: without one the result is
    invalid but flagged ``encrypted=True`` so the caller can prompt. With a
    wrong password the result is invalid with a clear error.
    ...
    if crypto.looks_encrypted(source_path):
        # A SQLCipher file is whole pages (size a multiple of 512, >= one
        # page). Anything else with no plaintext magic is just junk -- say
        # so rather than uselessly prompting for a password.
        size = source_path.stat().st_size
        if size < 512 or size % 512 != 0:
            return ValidationResult(
                ok=False, errors=["not a SQLite or SQLCipher database file"]
            )
        if not password:
            return ValidationResult(
                ok=False,
                errors=["encrypted backup — a password is required"],
                encrypted=True,
            )
        try:
            conn = crypto.open_connection(source_path, password)
        except crypto.WrongPassword:
            return ValidationResult(
                ok=False, errors=["incorrect backup password"], encrypted=True
            )
        except crypto.EncryptionUnavailable as e:
            return ValidationResult(ok=False, errors=[str(e)], encrypted=True)
        try:
            counts, errors = _check_required_tables(conn)
        finally:
            conn.close()
        return ValidationResult(
            ok=not errors, table_counts=counts, errors=errors, encrypted=True
        )

    # Plaintext path: fail fast on a non-SQLite upload (HTML error page, etc.).
    try:
...
def restore_database(
    ...
    Validates the source first (raises ``ValueError`` if invalid). An
    encrypted backup requires its ``password``; the restored database stays
    encrypted under that key, so the caller must record it as the active
    session password afterwards. If ``keep_safety`` is True (default) and a
    current DB exists, a timestamped copy is saved alongside it as
    ``<stem>.pre-restore.<unix_ts>.sqlite`` so the user can roll back.

    Caller MUST close any live connection to ``current_path`` before
    invoking this (e.g. ``conn.close()`` + clear streamlit's cache).
    Failing to do so on Windows raises ``PermissionError`` on the replace.
    """
    current_path = Path(current_path)
    source_path = Path(source_path)
    result = validate_backup(source_path, password=password)
    if not result.ok:
        raise ValueError(
    ...
    # Drop the replaced DB's stale -wal/-shm so they aren't applied on top
    # of the new file (which may use a different cipher/format).
    for suffix in ("-wal", "-shm"):
        try:
            Path(str(current_path) + suffix).unlink()
        except OSError:
            pass
