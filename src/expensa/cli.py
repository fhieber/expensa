...
from expensa import __version__
...
from expensa.utils.logging import configure_logging
...
def _global_home() -> Path:
    ...
    Honours ``$EXPENSA_HOME`` so tests can point us at a
...
def _resolve_account(
    ...
    if explicit:
        ...
        if match is None:
            raise click.ClickException(
                ...
                "Run `expensa account list` to see what's available."
            )
        return match
    ...
    # global_home so commands like `expensa init` keep working.
    ...
    if crypto.looks_encrypted(cfg.db_path):
        ...
        if not password:
            if sys.stdin.isatty():
                ...
                raise click.ClickException(
                    "database is encrypted: set EXPENSA_DB_PASSWORD "
...
def cli(
    ...
    """Expensa — local German expense analysis."""
...
def init(ctx: click.Context, name: str | None, with_defaults: bool) -> None:
    ...
    if not registry.all():
        ...
        from expensa.accounts import init_account_db
        try:
    ...
    try:
        if with_defaults:
            from expensa.storage.categories import import_categories_from_yaml
...
def categories_add(ctx: click.Context, name: str, description: str, color: str) -> None:
    ...
    conn = _connect(cfg)
    try:
        from expensa.storage.categories import upsert_category
...
def account_encrypt(
    ...
    if not info.db_path.is_file():
        raise click.ClickException(
            f"no database yet for {info.id!r}; run `expensa init` first."
        )
...
def account_decrypt(ctx: click.Context, name: str | None) -> None:
    ...
    Reads the current password from ``EXPENSA_DB_PASSWORD`` or
    ...
    pw = os.environ.get("EXPENSA_DB_PASSWORD") or click.prompt(
...
def account_passwd(ctx: click.Context, name: str | None) -> None:
    ...
    if not crypto.looks_encrypted(info.db_path):
        raise click.ClickException(
            f"{info.id!r} is not encrypted; use `expensa account encrypt` first."
        )
    old = os.environ.get("EXPENSA_DB_PASSWORD") or click.prompt(
        "Current password", hide_input=True
    )
...
def own_iban_list(ctx: click.Context) -> None:
    ...
    conn = _connect(cfg)
    try:
        from expensa.storage.own_ibans import list_own_ibans
...
def own_iban_add(ctx: click.Context, iban: str, label: str) -> None:
    ...
    conn = _connect(cfg)
    try:
        from expensa.storage.own_ibans import add_own_iban
...
def own_iban_remove(ctx: click.Context, iban: str, yes: bool) -> None:
    ...
    conn = _connect(cfg)
    try:
        from expensa.storage.own_ibans import remove_own_iban
...
def _adapter_and_records(path: Path, source: str):
    ...
    from expensa.ingestion._parsing import CsvParseError
    from expensa.ingestion.sources import detect_adapter, get_adapter
...
def _preview_enrich(
    ...
    from expensa.enrichment.secondary import preview_enrichment
...
def label(ctx: click.Context, n: int | None, strategy: str | None) -> None:
    ...
    try:
        ...
        cats = list_categories(conn)
        if not cats:
            click.echo("No categories yet. Run `expensa init` or `expensa categories add ...`.", err=True)
            ctx.exit(2)
...
def viz(ctx: click.Context, name: str, out: Path | None, since: str | None, until: str | None) -> None:
    ...
    conn = _connect(cfg)
    try:
        from expensa.viz import CHART_BUILDERS, save_figure
...
def vendor_lookup(ctx: click.Context, counterparty: str | None, all_vendors: bool) -> None:
    ...
    try:
        ...
        from expensa.ingestion.normalizer import normalize_counterparty
...
def vendor() -> None:
    ...
    Read-only. To populate or refresh the cache, use ``expensa vendor-lookup``.
    """
    ...
    try:
        ...
        if r is None:
            click.echo(
                ...
                f"`expensa vendor-lookup {counterparty!r}` to populate it.",
                err=True,
            )
            ctx.exit(1)
...
def _open_browser_when_ready(host: str, port: int, timeout: float = 30.0) -> None:
    """Open the browser once the server is up, from a background thread.

    Only safe when the calling process stays alive long enough for the
    thread to run (i.e. foreground mode). For detached mode the parent
    exits immediately, so use a synchronous ``_wait_for_server`` +
...
def ui(ctx: click.Context, foreground: bool, no_browser: bool) -> None:
    ...
    immediately. Stop it later with `expensa ui-stop`. The Streamlit
    ...
    """
    from expensa.ui.process import (
        UiProcessInfo,
        is_alive,
        read_pid_file,
        spawn_detached,
    ...
    env = os.environ.copy()
    env["EXPENSA_HOME"] = str(global_home)
    url = f"http://{cfg.streamlit.host}:{cfg.streamlit.port}"
    click.echo(f"-> {url}")

    if foreground:
        # The CLI process stays alive, so a background thread can wait
        # for readiness and open the browser without being killed.
        if not no_browser:
    ...
    click.echo("stop with: expensa ui-stop")

    # Detached: the parent process is about to exit, which would kill any
    # background browser-opener thread before the server is listening.
    # Wait synchronously (briefly) for readiness, then open the browser.
    if not no_browser:
        if _wait_for_server(cfg.streamlit.host, cfg.streamlit.port):
            import webbrowser

            webbrowser.open(url)
        else:
            click.echo(
                f"UI not reachable yet; open {url} manually once it's up.",
                err=True,
            )
