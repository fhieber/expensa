...
def test_vendor_label_distribution_restrict_ids(
    ...
    rows = tmp_db.execute(
        "SELECT id FROM expenses WHERE counterparty_normalized = 'markt alpha' ORDER BY id"
    ).fetchall()
    ids = [int(r["id"]) for r in rows]
    for eid in ids:
        add_label(tmp_db, eid, cid, "user")

    full = vendor_label_distribution(tmp_db, "markt alpha")
    assert full == {cid: len(ids)}

    # Restricting to a subset reduces the count; excluding all returns {}.
    subset = set(ids[:-1])
    restricted = vendor_label_distribution(tmp_db, "markt alpha", restrict_ids=subset)
    assert restricted == {cid: len(subset)}
    assert vendor_label_distribution(tmp_db, "markt alpha", restrict_ids=set()) == {}
...
def test_cascade_train_ids_prevents_vendor_leak(
    ...
    rent_row = int(
        tmp_db.execute(
            "SELECT id FROM expenses WHERE counterparty_normalized = 'vermieter' LIMIT 1"
        ).fetchone()["id"]
    )
    for eid in alpha_rows:
        add_label(tmp_db, eid, cid, "user")
    add_label(tmp_db, rent_row, other, "user")

    held_out = alpha_rows[0]
    train_ids = set(alpha_rows[1:]) | {rent_row}
    cascade = CategorizationCascade(
        tmp_db, _cfg(tmp_path), HashEmbedder(dim=64), train_ids=train_ids
    )
    cascade.fit()
    # Vendor match should still fire for the held-out Markt Alpha row because OTHER
    # Markt Alpha rows are in the train set -- that's legitimate, not a leak.
    pred = cascade.predict_batch([held_out])[0]
    assert pred.stage == "vendor_exact_match"
    assert pred.category_id == cid

    # But if NO Markt Alpha rows are in the train set, vendor match must not fire.
